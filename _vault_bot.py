...
#  НАСТРОЙКИ
# ══════════════════════════════════════════════
TELEGRAM_TOKEN  = "6829443456:AAF1z3FywACNpsBVJ2mdFgo_R_Z5iaVedFM"   # ← от @BotFather
ANTHROPIC_KEY   = "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"    # ← console.anthropic.com
VAULT_PATH      = "/Users/ods/Downloads/AI/ObsidianVault"            # ← путь к вашему vault
GROK_KEY        = "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"         # ← console.x.ai → API Keys
ALLOWED_USER_ID = 0
MY_EMAIL = 's.klevtsov@auchan.ru'
MY_NAME = 'Stanislav KLEVTSOV'



ALERT_KEYWORDS  = ["urgent", "срочно", "invoice", "счёт", "deadline", "дедлайн"]
DIGEST_WEEKDAY  = 0   # понедельник
DIGEST_HOUR     = 7   # 07:00 UTC

# Настройки Grok STT
GROK_STT_URL    = "https://api.x.ai/v1/stt"
GROK_CHAT_URL   = "https://api.x.ai/v1/chat/completions"
GROK_CHAT_MODEL = "grok-3-turbo"
GROK_LANGUAGE   = "ru"
...
GROQ_KEY        = "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"         # ← console.groq.com → API Keys
GROQ_CHAT_URL   = "https://api.groq.com/openai/v1/chat/completions"
GROQ_MODEL      = "llama-3.1-8b-instant"

# Доступные LLM для переключения
AVAILABLE_MODELS = {
    "haiku":  {"name": "Claude Haiku",        "provider": "anthropic", "model": "claude-haiku-4-5-20251001"},
    "llama":  {"name": "Llama 3.1 8B Instant","provider": "groq",      "model": "llama-3.1-8b-instant"},
}
DEFAULT_MODEL = "haiku"
# ══════════════════════════════════════════════
    ...
    def _call_llm(self, messages: list, system: str = "",
        ...
        if provider == "anthropic":
            ...
            if GROQ_KEY == "YOUR_GROQ_API_KEY":
                raise ValueError("Укажите GROQ_KEY в настройках скрипта")
            groq_msgs = []
            if system:
...
def grok_transcribe(audio_path: str) -> str:
    ...
    Поддерживает: ogg, mp3, wav, m4a, flac и др.
    """
    if GROK_KEY == "YOUR_GROK_API_KEY":
        raise ValueError("Укажите GROK_KEY в настройках скрипта")
    ...
    resp = requests.post(
        ...
        files={"file": (Path(audio_path).name, audio_bytes, mime)},
        data={
            "language": GROK_LANGUAGE,
            "temperature": "0",          # детерминированная транскрипция
        },
        timeout=60
    )
    resp.raise_for_status()
    ...
    # Grok STT возвращает {"text": "..."} или {"segments": [...]}
    if "text" in data:
        return data["text"].strip()
    elif "segments" in data:
        return " ".join(s.get("text","") for s in data["segments"]).strip()
    else:
        raise ValueError(f"Неожиданный ответ STT: {data}")
