...
#  НАСТРОЙКИ
# ══════════════════════════════════════════════
from config import YANDEX_DISK_TOKEN, VAULT_PATH, DISK_ROOT_FOLDER, MAX_FILE_SIZE_MB
# ══════════════════════════════════════════════
...
class YandexDiskAPI:
    ...
    def _req(self, method: str, url: str, **kwargs):
        """Выполняет запрос с обходом SSL проблем на macOS/Python 3.9."""
        import ssl, urllib3
        urllib3.disable_warnings()
        # Пробуем сначала нормально, потом с verify=False
        try:
            return requests.request(method, url, headers=self.headers,
                                    timeout=30, **kwargs)
        except requests.exceptions.SSLError:
            log.warning("SSL ошибка — пробую с verify=False")
            return requests.request(method, url, headers=self.headers,
                                    timeout=30, verify=False, **kwargs)

    def _check_token(self):
        resp = self._req("GET", f"{DISK_API}/")
    ...
    def download_file(self, disk_path: str) -> bytes:
        ...
        resp = self._req("GET", f"{DISK_API}/resources/download",
        ...
        try:
            ...
            file_resp = requests.get(url, timeout=60, verify=False)
